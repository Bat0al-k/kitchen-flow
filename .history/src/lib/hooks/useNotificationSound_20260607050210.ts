...
export function useNotificationSound() {
  const [isMuted, setIsMuted] = useState<boolean>(true); // Default to muted or load from local storage
  ...
  useEffect(() => {
    const saved = localStorage.getItem("kitchenflow_sound_muted");
    if (saved !== null) {
      setIsMuted(saved === "true");
    } else {
      // Default to unmuted if not previously customized
      setIsMuted(false);
    }
  }, []);
